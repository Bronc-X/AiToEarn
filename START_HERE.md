...
If Docker reports that `C:\ProgramData\DockerDesktop` must be owned by an elevated account, rerun the same command from an administrator PowerShell. The script repairs that folder owner, enables WSL and Virtual Machine Platform, then installs Docker Desktop.
